    ...
    <div className='container-lg px-0'>
      <div className='row'>
        <div className='col-auto order-1 me-auto'>
          <p className='fw-semibold m-0 todoTitle'>{title}</p> 
          <p className='my-1 small text-secondary todoDescription'>{description}</p>
        </div>
        <div className='col-sm-auto order-3 order-sm-2 mb-1 pe-0 small text-secondary'>
          Created: <span className='text-nowrap todoCreatedDate'>{createdDate.toISOString().split('T')[0]}</span>
        </div>
        <div className='col-auto order-2 mb-1 order-sm-3 text-end'>
          <div className='btn-group' aria-label='Toggle completed'>
            <button type='button' className={`btn btn-sm btn-outline-success btnCompleted ${completed && 'text-bg-success'}`}>
          ...
          <div className='row gx-2'>
            <div className='col-sm-auto'>
              <div className='d-flex align-items-center'>
                ...
                Due:<span className='ms-1 text-nowrap todoDueDate'>{dueDate.toISOString().split('T')[0]}</span>
              </div>
            </div>
            <div className='col-sm-auto mt-2 mt-sm-0 d-flex align-items-center gap-2'>
              ...
              {attachments &&
                <span className='badge text-bg-secondary text-white py-1 d-flex align-items-center todoAttachmentsBadge'>
                  ...
                  <span className='todoAttachements'>{attachments.length} attachments</span>
                </span>
              }
            </div>
          </div>
