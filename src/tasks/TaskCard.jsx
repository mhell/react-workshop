    ...
    <div className="card border shadow-sm mx-4 mt-0 mb-5" id="todoHolder">
      <div className="card-header bg-white d-flex justify-content-between align-items-center p-3">
        ...
        <div className="btn-group" aria-label="Filter">
          <button type="button" className={`btn btn-sm btn-outline-secondary btnFilter ${isFiltered && 'text-bg-secondary'}`} onClick={() => onFilter(isFiltered ? undefined : {completed: false})}>
            <i className="bi bi-funnel"></i>
          </button>
          <button type="button" className={`btn btn-sm btn-outline-secondary btnSort ${isSorted && 'text-bg-secondary'}`}>
